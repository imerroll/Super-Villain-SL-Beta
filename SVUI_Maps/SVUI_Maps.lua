...
if GetRealZoneText() == "Lunarfall" then
		return
	 end
if(WMP_XY_COORD and WorldMapFrame:IsShown()) then return end
	...
	if C_Map.GetBestMapForUnit("player") == 582 then
	
		return
	end
	if C_Map.GetBestMapForUnit("player") == 1473 then
	
		return
	end
	
	local MYzone = GetMinimapZoneText()
	local playerName = UnitName("player")
	if MYzone == playerName.."'s Garrison" then
		IsInInstance(true)
	end
	
	if GetMinimapZoneText() == UnitName("player").."'s Garrison" then
...
function MOD:Load()
	...
	self.Holder.Square = CreateFrame("Frame", nil, self.Holder, BackdropTemplateMixin and "BackdropTemplate")
 ...
 -- self.InfoTop:SetBackdropColor(1, 1, 0, 1)
	...
	if(SV.db.Maps.minimapbar.enable == true) then
		MMBHolder = CreateFrame("Frame", "SVUI_MiniMapButtonHolder", self.Holder, BackdropTemplateMixin and "BackdropTemplate")
		...
		MMBBar = CreateFrame("Frame", "SVUI_MiniMapButtonBar", MMBHolder, BackdropTemplateMixin and "BackdropTemplate")
		MMBBar:SetFrameStrata("LOW")

...
-- Multi Frame Details by Vervent
DOCK_EMBEDS["Details"] = function(self)
    ...
    for i=10, 1, -1 do
        ...
        if _G['DetailsBaseFrame'..i] ~= nil then
            maxInstance = i;
            break;
        end
    end
	
    width = width / maxInstance;

    for i=1, maxInstance do
        _G['DetailsBaseFrame'..i]:ClearAllPoints()
        _G['DetailsBaseFrame'..i]:SetParent(self)
        _G['DetailsBaseFrame'..i]:SetSize(width - 4, height - 4)
        if (i == 1) then
            _G['DetailsBaseFrame'..i]:SetPoint("BOTTOMLEFT", self, "BOTTOMLEFT", 0, 2)
        else
            _G['DetailsBaseFrame'..i]:SetPoint("TOPLEFT", _G['DetailsBaseFrame'..(i-1)], "TOPRIGHT", 2, 0)
        end
        _G['DetailsBaseFrame'..i]:SetMovable(false);
        if(_G['DetailsRowFrame'..i]) then
            _G['DetailsRowFrame'..i]:ClearAllPoints()
            _G['DetailsRowFrame'..i]:SetParent(self)
            _G['DetailsRowFrame'..i]:SetAllPoints(DetailsBaseFrame1)
            self.Framelink = _G['DetailsRowFrame'..i]
        else
            self.Framelink = _G['DetailsBaseFrame'..i]
        end
        _detalhes.move_janela_func = SV.fubar
    end
	-- if _G['Details_Streamer'] ~= nil then
		-- local DSwidth = SV.Dock.BottomRight.Window:GetWidth();
		-- local DSheight = Details_Streamer:GetHeight()
		-- if(Details_Streamer) then
			-- Details_Streamer:ClearAllPoints()
			-- Details_Streamer:SetParent(self)
			-- Details_Streamer:SetSize(DSwidth-10, DSheight)
			-- Details_Streamer:SetPoint("BOTTOM", self, "TOP", -5, 15)
			-- self.Framelink = Details_Streamer
			-- return true
		-- else
			-- return false
		-- end

...
local function CreateMacroToolButton(proName, proID, itemID)
	...
	--if proID == 202 then proName = GetSpellInfo(51306) end
...
local function LoadToolBarProfessions()
	...
	-- if(fishing ~= nil and (SV.db.Dock.dockTools.fishing)) then
		-- proName, _, _, _, _, _, proID = GetProfessionInfo(fishing)
		-- CreateMacroToolButton(proName, proID, fishing)
	-- end

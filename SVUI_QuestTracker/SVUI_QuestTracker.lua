...
local OBJECTIVE_GetStatus = function(self)
	if(not self.Status) then
		local status = CreateFrame("Frame", nil, self, BackdropTemplateMixin and "BackdropTemplate")
		status:SetPoint("TOPLEFT", self.Icon, "TOPRIGHT", 4, 0);
		status:SetPoint("BOTTOMRIGHT", self, "BOTTOMRIGHT", 0, 0);

		status.Bar = CreateFrame("StatusBar", nil, status, BackdropTemplateMixin and "BackdropTemplate");
		...
		local bgFrame = CreateFrame("Frame", nil, status.Bar, BackdropTemplateMixin and "BackdropTemplate")
...
local OBJECTIVE_HEADER_Get = function(self, index)
	if(not self.Rows[index]) then
		...
		local objective = CreateFrame("Frame", nil, self, BackdropTemplateMixin and "BackdropTemplate");
...
function MOD:Load()
	...
	local scrollFrame = CreateFrame("ScrollFrame", "SVUI_QuestTrackerScrollFrame", self.Docklet, BackdropTemplateMixin and "BackdropTemplate");
	...
	local scrollBar = CreateFrame("Slider", "SVUI_QuestTrackerScrollFrameScrollBar", scrollFrame, BackdropTemplateMixin and "BackdropTemplate");

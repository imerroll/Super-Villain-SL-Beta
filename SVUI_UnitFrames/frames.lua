...
CONSTRUCTORS["player"] = function(self, unit)
   ...
   -- self.Power.frequentUpdates = true
	...
	local AdditionalPower = CreateFrame('StatusBar', nil, self)
		...
		AdditionalPower:SetSize(20, 10)
		AdditionalPower:SetPoint('BOTTOM',0,-35)
		AdditionalPower:SetPoint('LEFT')
		AdditionalPower:SetPoint('RIGHT')
		 --CreateBackdrop(AdditionalPower)

		self.AdditionalPower = AdditionalPower
		local MaxMana = UnitPowerMax("player", 0)
		
		local ManaText = AdditionalPower:CreateFontString(nil, "OVERLAY", "TextStatusBarText") -- parent to last child to make sure it's on top
		ManaText:SetPoint("CENTER", AdditionalPower, 0,0) -- but anchor to the base element so it doesn't wiggle
		ManaText:SetFont("Interface\\Addons\\SVUI_!Core\\assets\\fonts\\Numbers.TTF", 10, "OUTLINE")
		ManaText:SetJustifyH("LEFT")
		ManaText:SetText("")
		self:Tag(ManaText, "[power:curmana]")
  ...
  --  self.Health.frequentUpdates = true
   ...
   -- self.Power.frequentUpdates = true
    ...
    MOD:CreatePortrait(self)
	 ...
	 self.Castbar = MOD:CreateCastbar(self, false, L["Target Castbar"], true, true, true, true)
	--self.Castbar = MOD:CreateCastbar(self, true, L["Target Castbar"], false, true, true, true)
    MOD:CreateAuraFrames(self, key, true)
    self.Afflicted = MOD:CreateAfflicted(self)
    self.RaidIcon = MOD:CreateRaidIcon(self)
